...
public class ItemFireBow extends Item
{
	...
	public boolean onItemUse(ItemStack stack, EntityPlayer player, World world, int x, int y, int z, int face, float x2, float y2, float z2)
	{
		...
		}else if(block.getIdFromBlock(block) == RegisterBlocks.campfireKindling.getIdFromBlock(RegisterBlocks.campfireKindling)){
			world.setBlock(x, y, z, RegisterBlocks.campfireLit);
		}else if(block.getIdFromBlock(block) == RegisterBlocks.campfireLit.getIdFromBlock(RegisterBlocks.campfireLit)){
